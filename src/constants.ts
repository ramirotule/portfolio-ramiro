export const PERSONAL_DATA = {
  name: "Ramiro Toulemonde",
  role: "Senior Frontend Developer",
  experience: "+5 years",
  location: "Santa Rosa, La Pampa, Argentina",
  stack: [
    "React.js",
    "React Native",
    "TypeScript",
    "Python",
    "Supabase",
    "n8n",
    "Git",
    "GitHub",
    "Vite",
    "Framer Motion",
    "Lucide React",
    "Material UI",
    "Storybook",
    "Jest",
    "React Testing Library",
    "Sass",
    "Tailwind CSS",
    "Styled Components",
  ],
  socials: {
    linkedin: "https://linkedin.com/in/ramirotule",
    github: "https://github.com/ramirotule",
    email: "ramirotoulemonde@gmail.com",
  },
  aiStack: {
    description:
      "Human-AI Collaboration: High-performance workflow with Claude Code & AntiGravity.",
    tools: ["Claude Code", "AntiGravity", "Stitch MCP"],
  },
  workflowTools: [
    "Messaging: Slack, Discord",
    "Project Management: Jira",
    "AI Tools: Antigravity, Claude Code",
    "Code Editor: VS Code",
  ],
};

export const PROJECTS = [
  {
    id: "stockautos",
    title: "STOCK AUTOS (SaaS)",
    description:
      "Automotive Management system with n8n automation and Supabase backend.",
    tech: ["n8n", "Supabase", "React"],
    link: "https://stockautos.vercel.app/",
    images: [
      "/src/assets/stockautos.png",
      "/src/assets/stockautos-dashboard.png",
    ],
  },
  {
    id: "alkila",
    title: "Alkila",
    description: "Real Estate Platform with local impact in Santa Rosa.",
    tech: ["React JS", "Tailwind CSS"],
    link: "https://alkila.vercel.app/",
    images: ["/src/assets/alkila.png"],
  },
  {
    id: "raminformatica",
    title: "RAM Informática",
    description: "Python + WhatsApp pricing engine integration.",
    tech: ["Python", "WhatsApp API"],
    link: "https://www.raminformatica.com.ar",
    images: [
      "/src/assets/raminformatica.png",
      "/src/assets/raminformatica-dashboard.png",
    ],
  },
  {
    id: "detakito",
    title: "deTaquito — Owner Portal",
    description: "React Native Performance showcase with Fabric Architecture.",
    tech: ["React Native", "Fabric", "MobX"],
    link: "https://detakito-landing.vercel.app/",
    images: [
      "/src/assets/detaquito.png",
      "/src/assets/detaquito-dashboard1.png",
    ],
  },
  {
    id: "controlgastos",
    title: "Control Gastos",
    description: "Responsive web app for personal finance management.",
    tech: ["React", "Tailwind", "Supabase"],
    link: "#",
    images: ["/src/assets/controlgastos.png"],
  },
];
